...
def data_collect(start, end, session):
    for location in CONFIG.location:
        ...
        while since < end:
            page = 1
            result = location_users(location, page, since.strftime('%Y-%m-%d'), until.strftime('%Y-%m-%d'))
            # 获取失败重试
            if not result:
                continue
            ...
            if 0 < result.get("total_count") <= 1000:
                while True:
                    ...
                    if page == 11:
                        break
                logger.info(f"{since.strftime('%Y-%m-%d')}——{until.strftime('%Y-%m-%d')} {location} Data collection is complete!")
